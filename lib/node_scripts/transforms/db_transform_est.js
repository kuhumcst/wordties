...
var transformVariants = function(csv, store) {
	...
	.on('data', function(data, index){
	  ...
	  if(store[data.synset_number] == null)
	    store[data.synset_number] = [label, gloss];
	  else {
		if(gloss != null) {
			gloss = (store[data.synset_number][1] != null) ? store[data.synset_number][1] + "; " + gloss : gloss;
			if(store[data.synset_number][1] != null && data.synset_number == 4801) console.log(data.synset_number +  ":" + data.gloss + ":" + gloss)
	    } else gloss = store[data.synset_number][1];
		
		store[data.synset_number] = [store[data.synset_number][0] + " " + label, gloss];
	  }
	  
	  if(synsets[data.synset_number] == null)
