...
var FROM_REL_PATH = argv.f,
    ...
    translationsToRemove = new Array(),
...
var runToWNCSV = function() {
	...
	.on('data', function(data, index){
	  ...
	  if(idx != -1 && translations[idx] != null) {
		if(Array.isArray(translations[idx])) { 
	...
	.on('end', function(count){
	  ...
	  for(var i in translations) queryTraverse(translations[i]);
	})
	.on('error', function(err) {
	  process.stderr.write(err.message);
	})
...
var retrieveEngGloss = function(client, hash) {
 	var query_alignments = 'SELECT definition FROM alignments a WHERE a.source_id = $1 AND a.key = $2';

	// PSQL query Alignments
   	//----------------------- 
	var pwnId = hash.valuesAt('pwn_id');	
	client.query(query_alignments, ['wordnet30', pwnId], function(err, result) {
		if(err) console.log(err);
		else {
		  var definition = "";
		  for(var i=0; i<result.rows.length; i++)
		  ...
		  console.log('CorePWN definition(' + pwnId + '): ' + definition);
...
var queryTraverse = function(translation) {
	if(Array.isArray(translation)) { 
	  ...
	  if(translation.has('wn_id_from') && translation.has('wn_id_to')) {
	    ...
	    var pwnId = translation.valuesAt('pwn_id');
	    console.log('Missing link to target found: ' + pwnId);
	    translationsToRemove.push(translation);
	  }
	}
}
...
var addToResult = function(result) {
	...
	} else if(translationsToRemove.indexOf(result) <= -1) {
		resultArray.push(result.values); 
	} else
	     console.log('result not added: ' + util.inspect(result.values, false));
