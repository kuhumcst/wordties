...
// FUNCTIONS
//-----------
// Postgres Query of AndreOrd DB for Gloss and Lemma data for from-target WN
var queryAndreOrd = function(syn_set_id, hash) {
	...
	client.query(query_join, [syn_set_id], function(err, result) {
	  if(err) { console.log(err); }
	    ...
	    if(hash != null)
	      hash.update({label_from: label, gloss: gloss}); 
	  }		
	})
...
var queryTraverse = function(translation) {
	if(translation instanceof Array) { 
	  ...
	  translation.forEach(function (x, key) {
	    //console.log(key + ' => ' + x);
	    if(key == 'wn_id_from') { 
	      queryAndreOrd(x*1000, translation);
	    }
    	  });
	}
}
